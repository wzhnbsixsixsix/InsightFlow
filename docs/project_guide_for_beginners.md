...
InsightFlow 是一个**自动寻找销售线索**的工具。

简单说：你输入一个产品名字（比如"碳化硅二极管"），它会自动帮你：

1. 上网搜索这个产品的信息
2. 分析哪些公司可能需要这个产品
3. 给这些公司打分，找出最有可能成交的客户
4. 搜索这些公司关键联系人的信息
5. 生成一份完整的报告（Markdown 文件 + CSV 表格）
...
└────────┬────────┘
         ▼
┌─────────────────┐
│ Stage 3:         │   市场扫描器按照搜索计划，并行搜索潜在客户
│ Market Scanner   │   输出：原始线索列表（RawLead[]）
│ （可能并行多个）  │   注意：这里会去重，同一个公司不重复
└────────┬────────┘
         ▼
┌─────────────────┐
│ Stage 4:         │   线索评估师用 BANT 框架给每个线索打分
│ Lead Qualifier   │   B=预算 A=权限 N=需求 T=时机（各25分，满分100）
│                  │   输出：Hot(>70) / Warm(40-70) / Cold(<40)
└────────┬────────┘
         ▼
┌─────────────────┐
│ Stage 5:         │   只针对 Hot 和 Warm 的线索，搜索联系人信息
│ Contact Enrich.  │   输出：联系人姓名、职位、邮箱等
└────────┬────────┘
         ▼
┌─────────────────┐
│ Stage 6:         │   报告撰写师把所有信息整合成一份报告
│ Report Writer    │   输出：Markdown 报告 + CSV 文件
└─────────────────┘
         │
         ▼
...
with gr.Blocks() as app:
    product_input = gr.Textbox(label="产品名称")     # 输入框
    depth = gr.Radio(choices=[...], label="搜索深度")  # 单选按钮
